// Integration test for PTX bindings format verification
...
fn test_ptx_bindings_format() {
    // This test verifies PTX generates correct binding types
    let temp_dir = env::temp_dir().join("bindgen_cuda_test_ptx_format");
    ...
    assert!(!bindings_content.contains("&[u8]"), 
            "PTX bindings should not use &[u8] type");
    assert!(!bindings_content.contains("include_bytes!"), 
            "PTX bindings should not use include_bytes! macro");
